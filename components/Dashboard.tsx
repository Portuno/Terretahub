    ...
    <div className="flex min-h-screen bg-[#F9F9F9]">
      ...
      <main className="flex-1 flex flex-col min-h-screen transition-all duration-300">
        ...
        {/* Top Navbar - Only show if not on Landing Page (index) 
            Actually, Landing Page in user prompt "debe ver el contenido de la landing pero ya dentro del marco de la app"
            So we should show navbar? Or Landing Page provides its own header?
            The LandingPage component has a huge Hero section.
            If I show navbar on Landing Page, it might duplicate with LandingPage content if LandingPage was designed as full page.
            Let's check LandingPage. It has "Terreta Hub" huge title.
            Maybe for "/" we hide the header or simplify it.
        */}
        {location.pathname !== '/' && (
          <header className="bg-white border-b border-gray-100 h-14 md:h-16 px-4 md:px-8 flex items-center justify-between sticky top-0 z-10">
